...
In particular, by using the `width` parameter, we can make the order of the string data match the numerical order (for example, output " 2" instead of "2" to come before "10" when data is presented). This can be important in some tools like *Power BI*.
...
If we compare the number of rows in the table with the original, we see that it does not match. The explanation is in the records that do not have defined values ​​in the considered variables that are eliminated when performing the operation. The difference is 17 rows which is the number of rows that have all undefined values, as shown below.

```{r, results = "asis"}
t <- l_mrs_ft[['mrs_age']] |>
  get_table()
t <- t[, c(12:16)]
t <- t[is.na(t[, 1]) & is.na(t[, 2]) & is.na(t[, 3]) & is.na(t[, 4]) & is.na(t[, 5]), ]

pander::pandoc.table(t, split.table = Inf)

cat(sprintf("Number of rows in the table: %d", nrow(t)))
```
