...
The separation is carried out by means of the `separate_measures()` function, in which we can indicate lists of measures and the name of each new flat table that will form the result. Groups are selected from the result using the `select_measure_group()` function.
...
mrs_cause_ft <- l_mrs_ft |>
  select_measure_group('mrs_cause')
mrs_age_ft <- l_mrs_ft |>
  select_measure_group('mrs_age')
```
