...
db_finest <- db_finest |>
  ...
  rolap::set_dimension_attribute_names(
    name = "where",
    attributes = c(
      "gas_station",
      "chain",
...
Next we are going to perform a multidimensional design with a coarser granularity. Let's assume that we are only interested in the hour in which the transaction occurs, not the specific minute: We are going to add an *Hour* field obtained from the *Time* field.
...
We have assumed that we are not interested in individual transactions but in a summary of them; neither the particular data of the clients, but only the associated segment and currency. Finally, we are not interested in gas stations either, but only the chain to which they belong and the rest of the dimension attributes.
