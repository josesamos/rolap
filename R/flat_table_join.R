...
#'
#' @export
lookup_table.flat_table <- function(ft, pk_attributes = NULL, attributes = NULL, attribute_agg = NULL, measures = NULL, measure_agg = NULL) {
  pk_attributes <- validate_attributes(ft$attributes, pk_attributes)
  ft$pk_attributes <- pk_attributes
  ft <- replace_empty_values(ft, pk_attributes)
  ...
  if (nrow(pk) < nrow(ft$table)) {
    if (length(pk_attributes) == length(ft$attributes) + length(ft$measures)) {
      ...
      if (!is.null(attributes)) {
        ...
        stopifnot("Each additional attribute must have an aggregation function." = length(attributes) == length(attribute_agg))
      }
      if (length(ft$attributes) > length(pk_attributes) + length(attribute_agg)) {
        stop("Attributes do not form a primary key. Aggregation functions (MAX, MIN) are needed for the rest of the attributes.")
      }
      if (!setequal(measures, ft$measures) | length(ft$measures) != length(measure_agg)) {
        stop("Attributes do not form a primary key. Aggregation functions (MAX, MIN, SUM, MEAN) are needed for measures.")
      }
      ...
      for (f in measure_agg) {
        stopifnot("Aggregation functions for measures must be MAX, MIN, SUM or MEAN." = f %in% c('MAX', 'MIN', 'SUM', 'MEAN'))
      }
      ft_group <- dplyr::group_by_at(as.data.frame(ft$table), dplyr::vars(tidyselect::all_of(pk_attributes)))
      agg <- list()
      measures <- c(attributes, ft$measures)
      agg_functions <- c(attribute_agg, measure_agg)
      for (i in seq_along(measures)) {
        if (agg_functions[i] == "MEAN") {
          df <- dplyr::summarize_at(ft_group, dplyr::vars(measures[i]), mean, na.rm = TRUE)
      ...
      res <- dplyr::ungroup(purrr::reduce(agg, dplyr::inner_join, by = pk_attributes))
...
#'
#' @export
join_lookup_table.flat_table <- function(ft, fk_attributes = NULL, lookup) {
  stopifnot("lookup does not include flat_table object." = methods::is(lookup, "flat_table"))
  stopifnot("lookup does not have a primary key defined." = length(lookup$pk_attributes) > 0)
  if (is.null(fk_attributes)) {
    fk_attributes <- lookup$pk_attributes
  }
  fk_attributes <- validate_attributes(ft$attributes, fk_attributes)
  stopifnot("The foreign and primary keys do not match." = length(lookup$pk_attributes) == length(fk_attributes))
  pk <- unique(lookup$table[, lookup$pk_attributes])
  stopifnot("The lookup table has probably changed since its definition." = nrow(lookup$table) == nrow(pk))

  ft <- replace_empty_values(ft, fk_attributes)
  rest <- setdiff(c(lookup$attributes, lookup$measures), lookup$pk_attributes)
  ...
  ft$table <- dplyr::left_join(ft$table, t, by = fk_attributes, suffix = c("", ".lookup"))
